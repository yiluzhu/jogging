...
class TestUserCreation(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.params = {
            'forename': 'Jeffrey', 'surname': 'Wood', 'username': 'jeffreywood',
            'password': '7e24', 'email': 'jeffrey.wood@gmail.com', 'role': 'user'
        }
    ...
    def test_create_user_unauthenticated(self):
        resp = self.client.put('/user', data=json.dumps(self.params), headers={'content-type': 'application/json', 'Authorization': 'dummy token'})
        self.assertEqual(resp.status_code, 401)

    def test_create_user_no_access(self):
        resp = self.client.put('/user', data=json.dumps(self.params), headers={'content-type': 'application/json', 'Authorization': self.user_token})
        self.assertEqual(resp.status_code, 403)

    def test_create_duplicate_user(self):
        params = {'forename': 'Tony', 'surname': 'Foltz', 'username': 'tonyfoltz', 'password': '7e24', 'email': 'tony.foltz@gmail.com'}
        resp = self.client.put('/user', data=json.dumps(params), headers={'content-type': 'application/json', 'Authorization': self.admin_token})
        self.assertEqual(resp.status_code, 409)

    def test_create_user_admin_role(self):
        resp = self.client.put('/user', data=json.dumps(self.params), headers={'content-type': 'application/json', 'Authorization': self.admin_token})
